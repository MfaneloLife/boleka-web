...
import BankingDetailsForm from "@/src/components/BankingDetailsForm";
...
export default function WalletPage() {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      setWithdrawMessage({ type: "success", text: "Withdrawal initiated. Funds will be sent to your bank account." });
      fetchData();
    } catch (err) {
      setWithdrawMessage({
        type: "error",
        text: err instanceof Error ? err.message : "Failed to withdraw",
      });
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-orange-500 to-amber-500 rounded-2xl p-6 text-white shadow-lg">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button
            ...
            disabled={withdrawLoading || !wallet || wallet.availableBalance <= 0}
            className="flex items-center justify-center gap-1.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm px-4 py-2 rounded-xl text-sm font-medium transition disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {withdrawLoading ? (
      ...
      {wallet && (
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
            <p className="text-lg font-bold text-gray-900">
              R{wallet.completedSalesTotal.toFixed(2)}
            </p>
            <p className="text-xs text-gray-500 mt-0.5">Total Sales</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-100 p-4 text-center">
            <p className="text-lg font-bold text-gray-900">
              R{wallet.paidOutTotal.toFixed(2)}
            </p>
